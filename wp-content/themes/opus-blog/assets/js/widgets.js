jQuery(document).ready(function ($) {
    ...
    $( '.opus-blog-install-plugins' ).click( function ( e ) {
        e.preventDefault();

        $( this ).addClass( 'updating-message' );
        $( this ).text( opus_blog_adi_install.btn_text );

        $.ajax({
            type: "POST",
            url: ajaxurl,
            data: {
                action     : 'opus_blog_getting_started',
                security : opus_blog_adi_install.nonce,
                ...
                setTimeout(function(){
                    $.ajax({
                        ...
                        success:function( response ) {
                            ...
                            redirect_uri         = opus_blog_adi_install.adminurl+'/themes.php?page=advanced-import&browse=all';
